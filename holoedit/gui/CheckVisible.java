...
public class CheckVisible extends JToggleButton implements ActionListener
{
	/* numero de la piste */
	private int numero;
	private TrackSelector tks;
	private static ImageIcon onIcon = new ImageIcon("./images/on.gif");
	private static ImageIcon offIcon = new ImageIcon("./images/off.gif");
	
	public CheckVisible(int numero, TrackSelector tks)
	{
		this.numero = numero;
		this.tks = tks;
		this.setSize(35, 20);
		this.setIcon(onIcon);
		this.setToolTipText("on/off");
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		if (mod == 24) {
			...
			if (!isSelected())
				tks.gp.viewAllButOne(getNumero());
				// sinon Solo
			else
				tks.gp.viewSolo(getNumero());
			...
			{
				tks.gp.viewAll();
				tks.gp.selectTrack(getNumero());
				// Aucune
			} else
				tks.gp.viewNone();
			// changer la visibilite de la piste >>>> click <<<< sans touche
		} else
			tks.gp.changeVisible(getNumero());
