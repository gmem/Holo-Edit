...
//		popupData.add(new JSeparator(SwingConstants.HORIZONTAL));
		...
		soundTree.addMouseListener(new MouseAdapter() {
			public void mousePressed(MouseEvent e) {
				// La ligne suivante est imp�rative pour l'affichage du jpopupmenu sous XP				
				...
				if ((e.isControlDown() && Ut.MAC) || e.getButton() == MouseEvent.BUTTON3) {
					...
					// remove autoris� si le noeud n'est pas la racine
		...
		dataTree.addMouseListener(new MouseAdapter() {
			public void mousePressed(MouseEvent e) {
				// les 2 lignes suivantes sont imp�ratives pour l'affichage correct du jpopupmenu sous XP
		...
		removeDtMI.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (result == 0) {
					...
					// si la data a remover est selectionn�e dans le sound tree
...
/**
 * 
 * @param toSelec Si un noeud particulier doit etre s�lectionn�.
 * 
 */
			...
			if (tr.isDataFlavorSupported(DataFlavor.javaFileListFlavor)) {
				...
				try {
					...
					if (droppedExternalData.getFileType().equalsIgnoreCase("SDIF") || droppedExternalData.getFileType().equalsIgnoreCase("TXT")) {
						...
						// on v�rifie que l'enfant n'existe pas deja
						if (!wave.containsLinkedData(hsdifdt)) {
							wave.addLinkedData(hsdifdt);
							// on d�-selectionne le noeud du dataTree :
